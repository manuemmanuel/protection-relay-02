...
export default function DCDetails() {
  ...
  const colors = {
    'Voltage': 'rgba(255, 206, 86, 1)', // yellow
    'Current': 'rgba(54, 162, 235, 1)',  // blue
  }
  ...
  const saveHistoricalData = async (data: RealTimeData) => {
    try {
      console.log('Attempting to save historical data:', {
        ...
        dc_voltage: data["DC Voltage"],
        dc_current: data["DC Current"],
          ...
          async (payload) => {
            ...
            if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
              ...
              setRealTimeData(current => {
                ...
                const hasChange = !lastData ||
                  Math.abs(newData["DC Voltage"] - lastData["DC Voltage"]) > threshold ||
                  Math.abs(newData["DC Current"] - lastData["DC Current"]) > threshold
      ...
      try {
        ...
        if (!error && data && isSubscribed) {
          setRealTimeData(current => {
            ...
            const hasChange = !lastData ||
              Math.abs(data["DC Voltage"] - lastData["DC Voltage"]) > threshold ||
              Math.abs(data["DC Current"] - lastData["DC Current"]) > threshold
    ...
    const datasets = [
      {
        ...
        data: sortedTimePoints.map(t => ({
          x: t,
          y: latestData["DC Voltage"]
        })),
        borderColor: colors['Voltage'],
        backgroundColor: colors['Voltage'].replace('1)', '0.5)'),
        tension: 0.4
      },
      {
        label: 'DC Current',
        data: sortedTimePoints.map(t => ({
          x: t,
          y: latestData["DC Current"]
        })),
        borderColor: colors['Current'],
        backgroundColor: colors['Current'].replace('1)', '0.5)'),
        tension: 0.4
      }
    ]
          ...
          <div className="flex flex-col xs:flex-row justify-between items-start xs:items-center gap-3 xs:gap-4">
            <div className="space-y-1.5 xs:space-y-2 w-full xs:w-auto">
              ...
              <p className="text-xs xs:text-sm sm:text-base text-gray-400">
                Real-time DC voltage and current measurements
              </p>
            </div>
              ...
              <>
                ...
                <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 xs:gap-4">
                  <div className="bg-gray-900/50 p-3 xs:p-4 sm:p-6 rounded-lg xs:rounded-xl border border-gray-800 
                    ...
                    <h3 className="text-sm xs:text-base sm:text-lg lg:text-xl font-semibold mb-3 xs:mb-4 
                      flex items-center gap-1.5 xs:gap-2" style={{color: colors['Voltage']}}>
                      <div className="w-1.5 h-1.5 xs:w-2 sm:w-3 xs:h-2 sm:h-3 rounded-full animate-pulse" 
                        style={{backgroundColor: colors['Voltage']}}/>
                      DC Voltage
                    </h3>
                    <div className="space-y-2 xs:space-y-3 sm:space-y-4">
                      <div className="flex justify-between items-center p-2 xs:p-3 bg-gray-800/40 rounded-lg">
                        <span className="text-xs xs:text-sm sm:text-base text-gray-400">Value</span>
                        <span className="text-xs xs:text-sm sm:text-base text-gray-100 font-mono 
                          ...
                          {realTimeData[0]?.["DC Voltage"].toFixed(2)} V
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-900/50 p-3 xs:p-4 sm:p-6 rounded-lg xs:rounded-xl border border-gray-800 
                    backdrop-blur-sm hover:border-gray-700 transition-colors">
                    <h3 className="text-sm xs:text-base sm:text-lg lg:text-xl font-semibold mb-3 xs:mb-4 
                      flex items-center gap-1.5 xs:gap-2" style={{color: colors['Current']}}>
                      <div className="w-1.5 h-1.5 xs:w-2 sm:w-3 xs:h-2 sm:h-3 rounded-full animate-pulse" 
                    ...
                    <div className="space-y-2 xs:space-y-3 sm:space-y-4">
                      <div className="flex justify-between items-center p-2 xs:p-3 bg-gray-800/40 rounded-lg">
                        ...
                        <span className="text-xs xs:text-sm sm:text-base text-gray-100 font-mono 
                          bg-gray-800/80 px-1.5 xs:px-2 sm:px-3 py-1 xs:py-1.5 rounded-lg">
                          {realTimeData[0]?.["DC Current"].toFixed(2)} A
                        </span>
                      </div>
                    </div>
                  </div>
                      ...
                      <tbody>
                        {historicalData.map((item) => (
                          ...
                          >
                            ...
                            <td className="p-2">{item.dc_voltage.toFixed(2)}</td>
                            <td className="p-2">{item.dc_current.toFixed(2)}</td>
